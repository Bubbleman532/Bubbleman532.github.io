	...
	function StateViz(container, nodes, linkArray) {
	  ...
	  */
	  ...
	  function dragstart(d) {
	    ...
	    svg.transition()
	      .style('box-shadow', 'inset 0 0 1px gold'); //yellow around canvas
	  }
	  function dragend() {
	    svg.transition()
	  ...
	  var force = d3.forceSimulation()
	    ...
	    .force('theta', d3.forceManyBody().theta(0.1))
	  ...
	  // Edges TODO pick up from here if possible
	  var edgeCounter = new EdgeCounter(linkArray);
	
	  var edgeselection = d3.select(svg)
	    .selectAll('.edgepath')
	    .data(linkArray)
	    .enter();
